...
// Paged Flash Attention kernel (fused version) integrated with GPU KV Cache
template <typename T>
__global__ void paged_flash_attention_kernel_fused(
    ...
    for (int tile_id = 0; tile_id < T; ++tile_id) {
        ...
        if (rerank_scores) {
            rerank_scores[batch_idx * T + tile_id] = logit;
        }
...
template __global__ void paged_flash_attention_kernel_fused<float>(
    const float*, KVTileCache<float>*, float*, float*, int, int, int, int,
